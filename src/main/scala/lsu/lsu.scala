...
import boom.util.{ AgePriorityEncoder, BoolToChar, GetNewBrMask, IsKilledByBranch, IsOlder, WrapInc }
 ...
 * @param pl_width pipeline width of the processor
 */
class LoadStoreUnit(pl_width: Int)(implicit p: Parameters, edge: freechips.rocketchip.tilelink.TLEdgeOut)
  ...
  var next_live_store_mask = Mux(clear_store, live_store_mask & ~(1.U << stq_head), live_store_mask)
  ...
  for (w <- 0 until pl_width) {
    ...
    ld_enq_idx = Mux(io.dec_ld_vals(w), WrapInc(ld_enq_idx, NUM_LDQ_ENTRIES), ld_enq_idx)
    ...
    next_live_store_mask = Mux(io.dec_st_vals(w), next_live_store_mask | (1.U << st_enq_idx), next_live_store_mask)
    st_enq_idx = Mux(io.dec_st_vals(w), WrapInc(st_enq_idx, NUM_STQ_ENTRIES), st_enq_idx)
  }
  ...
  val exe_tlb_uop = Mux(
    will_fire_sta_retry,
    stq_uop(stq_retry_idx),
    Mux(will_fire_load_retry, laq_uop(laq_retry_idx), io.exe_resp.bits.uop)
  )

  val exe_vaddr = Mux(
    will_fire_sta_retry,
    saq_addr(stq_retry_idx),
    Mux(will_fire_load_retry, laq_addr(laq_retry_idx), io.exe_resp.bits.addr.asUInt)
  )

  val dtlb = Module(
    new rocket.TLB(instruction = false, lgMaxSize = log2Ceil(coreDataBytes), rocket.TLBConfig(dcacheParams.nTLBEntries))
  )
  ...
  val mem_xcpt_valid = RegNext(
    ((pf_ld || pf_st || ae_ld || ae_st) ||
    ...
    Mux(
      ...
      Mux(
        ...
        Mux(
          ...
          Mux(ae_ld, rocket.Causes.load_access.U, rocket.Causes.store_access.U)
        )
      )
    )
  )
  ...
  assert(
    !(io.exe_resp.bits.mxcpt.valid && io.exe_resp.valid &&
  ...
  when(
    stq_allocated(stq_execute_head) &&
  ...
  assert(
    !(can_fire_store_commit && saq_is_virtual(stq_execute_head)),
  ...
  assert(
    PopCount(VecInit(will_fire_store_commit, will_fire_load_incoming, will_fire_load_retry, will_fire_load_wakeup))
      <= 1.U,
    "Multiple requestors firing to the data cache."
  )
  ...
  when(will_fire_load_incoming || will_fire_load_retry) {
    ...
    assert(
      !(will_fire_load_incoming && laq_addr_val(exe_tlb_uop.ldq_idx)),
  ...
  when(will_fire_sta_incoming || will_fire_sta_retry) {
    ...
    assert(
      !(will_fire_sta_incoming && saq_val(exe_tlb_uop.stq_idx)),
  ...
  when(will_fire_std_incoming) {
    ...
    assert(!(will_fire_std_incoming && sdq_val(sidx)), "[lsu] incoming store is overwriting a valid data entry.")
  }
  ...
  assert(
    !(io.fp_stdata.valid && io.exe_resp.valid && io.exe_resp.bits.uop.ctrl.is_std &&
  ...
  val mem_ld_uncacheable =
    Mux(RegNext(will_fire_load_wakeup), RegNext(laq_is_uncacheable(exe_ld_idx_wakeup)), mem_tlb_uncacheable)
  ...
  io.mem_ldSpecWakeup.valid := RegNext(
    will_fire_load_incoming
  ...
  val stdf_clr_bsy_valid = RegNext(
    mem_fired_stdf &&
  ...
  val do_stld_search = RegNext(
    (will_fire_sta_incoming ||
      will_fire_sta_retry),
    init = false.B
  )
  val do_ldld_search = RegNext(
    (will_fire_load_incoming ||
      will_fire_load_retry ||
      will_fire_load_wakeup),
    init = false.B
  ) && MCM_ORDER_DEPENDENT_LOADS.B
    ...
    when(do_stld_search) {
      ...
      when((laq_st_dep_mask(i) & (1.U << stq_idx)) =/= 0.U) {
        ...
        // NOTE: this address check doesn't necessarily have to be across all address bits
          .elsewhen(
            (lcam_addr(corePAddrBits - 1, 3) === l_addr(corePAddrBits - 1, 3)) &&
              l_allocated &&
              l_addr_val &&
              !l_is_virtual &&
              l_executed
...
//   ++ failed_loads)).asUInt)
  val temp_bits =
    (VecInit(VecInit.tabulate(NUM_LDQ_ENTRIES)(i => failed_loads(i) && i.U >= laq_head) ++ failed_loads)).asUInt
  ...
  val mem_xcpt_uop =
    Mux(mem_xcpt_valid, mem_tlb_uop, laq_uop(Mux(l_idx >= NUM_LDQ_ENTRIES.U, l_idx - NUM_LDQ_ENTRIES.U, l_idx)))
  ...
  for (i <- 0 until NUM_STQ_ENTRIES) {
    ...
    assert(
      !(IsKilledByBranch(io.brinfo, stq_uop(i)) && stq_allocated(i) && stq_committed(i)),
  ...
  for (w <- 0 until pl_width) {
    ...
    temp_stq_commit_head =
      Mux(io.commit_store_mask(w), WrapInc(temp_stq_commit_head, NUM_STQ_ENTRIES), temp_stq_commit_head)
  }
  ...
  when(stq_allocated(stq_head) && stq_committed(stq_head)) {
    clear_store := Mux(stq_uop(stq_head).is_fence, io.dmem_is_ordered, stq_succeeded(stq_head))
  }
  ...
  if (DEBUG_PRINTF_LSU) {
    printf("LSU:\n")
    printf("    WakeupIdx:%d LDatROBHead:%c\n", exe_ld_idx_wakeup, BoolToChar(io.commit_load_at_rob_head, 'V'))
    for (i <- 0 until NUM_LDQ_ENTRIES) {
      val t_laddr = laq_addr(i)
      printf(
        "    LDQ[%d]: State:(%c%c%c%c%c%c%c%d) STDep:(StqIdx:%d,Msk:%x) Addr:0x%x H,T:(%c %c)\n",
        i.U(ldqAddrSz.W),
    ...
    for (i <- 0 until NUM_STQ_ENTRIES) {
      ...
      printf(
        "    SAQ[%d]: State:(%c%c%c%c%c%c%c) BMsk:0x%x (Addr:0x%x -> Data:0x%x) H,ExH,CmH,T:(%c %c %c %c)\n",
        i.U(stqAddrSz.W),
...
object LoadDataGenerator {
  def apply(data: UInt, mem_size: UInt, mem_signed: Bool): UInt = {
    ...
    val out = Mux(
      ...
      Mux(
        word,
        Cat(Fill(32, sext & data(31)), data(31, 0)),
        Mux(
          half,
          Cat(Fill(48, sext & data(15)), data(15, 0)),
          Mux(byte_, Cat(Fill(56, sext & data(7)), data(7, 0)), data)
        )
      )
    )
    out
...
class ForwardingAgeLogic(num_entries: Int)(implicit p: Parameters) extends BoomModule()(p) {
  ...
  matches := Cat(io.addr_matches & age_mask.asUInt, io.addr_matches)
