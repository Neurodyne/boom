...
class LoadReqSlot(implicit p: Parameters) extends BoomModule()(p) {
  ...
  when(io.wen) {
    uop.br_mask := Mux(io.brinfo.valid, (io.in_uop.br_mask & ~io.brinfo.mask), (io.in_uop.br_mask))
 ...
 */
class DCacheShim(implicit p: Parameters)
  ...
  when(enq_rdy) { full_counter := 0.U }.otherwise { full_counter := full_counter + 1.U }
  ...
  val s1_stdata = RegNext(
    new freechips.rocketchip.rocket.StoreGen(io.core.req.bits.uop.mem_size, 0.U, io.core.req.bits.data, coreDataBytes).data
  )
  ...
  assert(
    !(io.core.resp.valid && RegNext(io.dmem.s2_xcpt.ma.ld) &&
