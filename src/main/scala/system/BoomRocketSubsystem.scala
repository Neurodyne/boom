...
trait HasBoomAndRocketTiles
  ...
  val rocketTiles = rocketTileParams.zip(rocketCrossings).map {
    case (tp, crossing) =>
      ...
      def treeNode: RocketTileLogicalTreeNode =
        new RocketTileLogicalTreeNode(rocket.rocketLogicalTree.getOMInterruptTargets)
      LogicalModuleTree.add(logicalTreeNode, rocket.rocketLogicalTree)

      rocket
  }

  val boomTiles = boomTileParams.zip(boomCrossings).map {
    case (tp, crossing) =>
      ...
      def treeNode: RocketTileLogicalTreeNode =
        new RocketTileLogicalTreeNode(boom.rocketLogicalTree.getOMInterruptTargets)
  ...
  val boomAndRocketTiles = (rocketTiles ++ boomTiles).sortWith(_.tileParams.hartId < _.tileParams.hartId).map { tile =>
    {
...
trait HasBoomAndRocketTilesModuleImp extends HasTilesModuleImp with HasPeripheryDebugModuleImp {
  val outer: HasBoomAndRocketTiles
}

class BoomRocketSubsystem(implicit p: Parameters) extends BaseSubsystem with HasBoomAndRocketTiles {
...
class BoomRocketSubsystemModuleImp[+L <: BoomRocketSubsystem](_outer: L)
    extends BaseSubsystemModuleImp(_outer)
    with HasResetVectorWire
    with HasBoomAndRocketTilesModuleImp {
  tile_inputs.zip(outer.hartIdList).foreach {
    case (wire, i) =>
      wire.hartid := i.U
