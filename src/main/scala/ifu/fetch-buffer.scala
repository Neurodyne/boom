 ...
 */
class FetchBuffer(numEntries: Int)(implicit p: Parameters)
    extends BoomModule
  ...
  val might_hit_head = (1 until fetchWidth)
    .map(
      k =>
        VecInit(rotateLeft(tail, k).asBools.zipWithIndex.filter { case (e, i) => i % coreWidth == 0 }.map {
          case (e, i) => e
        }).asUInt
    )
    .map(tail => head & tail)
    ...
    .orR
  val at_head = (VecInit(
    tail.asBools.zipWithIndex.filter { case (e, i) => i % coreWidth == 0 }.map { case (e, i) => e }
  ).asUInt & head).orR
  ...
  val will_hit_tail = (VecInit(
    (0 until numEntries)
      .map(
        i =>
          if (i % coreWidth == 0) false.B
          else head(i / coreWidth)
      )
  ).asUInt & tail).orR
  val at_tail = at_head
  ...
  if (DEBUG_PRINTF) {
    ...
    printf(
      "    Fetch3: Enq:(V:%c Msk:0x%x PC:0x%x) Clear:%c\n",
    ...
    printf("    Fetch4: Deq:(V:%c PC:0x%x)\n", BoolToChar(io.deq.valid, 'V'), io.deq.bits.uops(0).bits.pc)
  }
}
