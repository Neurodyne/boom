 ...
 */
class FetchTargetQueue(num_entries: Int)(implicit p: Parameters) extends BoomModule with HasBoomCoreParameters {
  ...
  when(io.deq.valid || io.flush.valid) {
    assert(
      !(io.deq.valid && io.flush.valid && io.deq.bits =/= io.flush.bits.ftq_idx),
  ...
  when((io.brinfo.valid && io.brinfo.mispredict) || io.flush.valid) {
    ...
    val new_ptr = WrapInc(Mux(io.flush.valid, io.flush.bits.ftq_idx, io.brinfo.ftq_idx), num_entries)
  ...
  when(do_deq) {
    ...
    if (DEBUG_PRINTF) {
      ...
      printf(
        "    Dequeue: Ptr:%d FPC:0x%x Hist:0x%x BIM:(Idx:(%d=%x) V:%c Mispred:%c Taken:%c CfiIdx:%d CntVal:%d CfiType:%c%c%c%c\n",
        deq_ptr.value,
        com_data.fetch_pc,
        com_data.history,
  ...
  assert(
    !(io.flush.valid && RegNext(io.com_ftq_idx) =/= io.flush.bits.ftq_idx),
  ...
  if (DEBUG_PRINTF && DEBUG_PRINTF_FTQ) {
    printf(
      "    Enq:(V:%c Rdy:%c Idx:%d) Commit:(V:%c Idx:%d) BRInfo:(V&Mispred:%c Idx:%d) Enq,Cmt,DeqPtrs:(%d %d %d)\n",
