...
import chisel3.{ withReset, DontCare }
...
import boom.exu._
import boom.util.{ AgePriorityEncoder, BoolToChar, ElasticReg }
 ...
 */
class FetchControlUnit(implicit p: Parameters) extends BoomModule with HasL1ICacheBankedParameters {
  ...
  val fb =
    if (useNewFetchBuffer) Module(new FetchBuffer(numEntries = fetchBufferSz * coreWidth))
      ...
      Mux(
        ...
        Mux(
          ...
          Mux(
            br_unit.take_pc,
            br_unit.target,
            Mux(r_f4_valid && r_f4_req.valid, r_f4_req.bits.addr, io.f2_btb_resp.bits.target)
          )
        )
      )
    )
...
//  val f3_bim_target = br_targs(f3_bim_br_idx)
  ...
  val f3_bpd_redirect_cfiidx =
    Mux(f3_bpd_may_redirect_taken, f3_bpd_br_idx, Mux(f3_has_jal, f3_jal_idx, (fetchWidth - 1).U))
  val f3_bpd_redirect_target =
    Mux(f3_bpd_may_redirect_taken, f3_bpd_target, Mux(f3_has_jal, f3_jal_target, nextFetchStart(f3_aligned_pc)))
  ...
  when(f3_valid && f4_ready && !r_f4_req.valid) {
    val last_idx = Mux(inLastChunk(f3_fetch_bundle.pc) && icIsBanked.B, (fetchWidth / 2 - 1).U, (fetchWidth - 1).U)
    ...
    prev_nextpc := alignToFetchBoundary(f3_fetch_bundle.pc) + Mux(
      inLastChunk(f3_fetch_bundle.pc) && icIsBanked.B,
      bankBytes.U,
      fetchBytes.U
    )
  }
  ...
  assert(
    PopCount(VecInit(f3_bpd_may_redirect_taken, f3_bpd_may_redirect_next)) <= 1.U,
  ...
  f3_btb_mask := Mux(f3_btb_resp.valid && !f3_req.valid, f3_btb_resp.bits.mask, Fill(fetchWidth, 1.U(1.W)))
...
//    }
  ...
  assert(!(r_f4_req.valid && !r_f4_valid), "[fetch] f4-request is high but f4_valid is not.")
  assert(
    !(io.clear_fetchbuffer && !(br_unit.take_pc || io.flush_take_pc || io.sfence_take_pc)),
    ...
    when(fb.io.enq.fire()) {
      ...
      for (i <- 0 until fetchWidth) {
        when(bundle.mask(i)) {
          ...
          printf(
            "%d; O3PipeView:fetch:%d:0x%x:0:%d:DASM(%x)\n",
    ...
    when(last_valid) {
      ...
      when(last_cfi_type === CfiType.none) {
        ...
        .elsewhen(last_cfi_type === CfiType.branch) {
          ...
          assert(fetch_pc === last_nextlinepc || f_pc === targ, "[fetch] branch is followed by the wrong instruction.")
        }
        .otherwise {
  ...
  when(
    ...
    val f3_is_jal = VecInit(f3_fetch_bundle.insts map { x =>
      GetCfiType(ExpandRVC(x)) === CfiType.jal
    }).asUInt & f3_fetch_bundle.mask
  ...
  if (DEBUG_PRINTF) {
    ...
    printf("    Fetch1:\n")
    printf(
      "        BRUnit: V:%c Tkn:%c Mispred:%c F0Redir:%c TakePc:%c RedirPc:0x%x\n",
      ...
      Mux(io.flush_take_pc, Str("F"), Mux(io.br_unit.take_pc, Str("B"), Str(" "))),
      f0_redirect_pc
    )
    ...
    printf("    Fetch2:\n")
    printf(
      "        IMemResp: V:%c Rdy:%c PC:0x%x Msk:0x%x\n",
    ...
    printf(
      "        IMemResp: BTB: Tkn:%c Idx:%d TRG:0x%x\n",
