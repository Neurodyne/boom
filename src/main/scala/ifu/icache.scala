...
import freechips.rocketchip.rocket.{ HasL1ICacheParameters, ICacheErrors, ICacheParams, ICacheReq }
 ...
 */
class ICache(val icacheParams: ICacheParams, val hartId: Int, val enableBlackBox: Boolean = false)(
 ...
 * @param outer top level ICache class
 */
abstract class ICacheBaseModule(outer: ICache) extends LazyModuleImp(outer) with HasL1ICacheBankedParameters {
...
class ICacheModule(outer: ICache) extends ICacheBaseModule(outer) {
  ...
  def addrMaybeInScratchpad(addr: UInt) =
    scratchpadBase
      .map(
        base =>
          addr >= base &&
            addr < base + outer.size.U
      )
      .getOrElse(false.B)
  def addrInScratchpad(addr: UInt) =
    addrMaybeInScratchpad(addr) &&
  ...
  if (cacheParams.fetchBytes <= 8) {
    ...
    val dataArrays = (0 until nWays).map { x =>
      SyncReadMem(nSets * refillCycles, UInt(dECC.width(wordBits).W)).suggestName("dataArrayWay_" + x.toString)
    }
    ...
    for ((dataArray, i) <- dataArrays zipWithIndex) {
      ...
      val mem_idx = Mux(
        ...
        Mux(s3_slaveValid, row(s1s3_slaveAddr), Mux(s0_slaveValid, row(s0_slaveAddr), row(s0_vaddr)))
      )
      when(wen) {
    ...
    val dataArraysB0 = (0 until nWays).map { x =>
      SyncReadMem(ramDepth, UInt(dECC.width(wordBits / nBanks).W)).suggestName("dataArrayB0Way_" + x.toString)
    }
    val dataArraysB1 = (0 until nWays).map { x =>
      SyncReadMem(ramDepth, UInt(dECC.width(wordBits / nBanks).W)).suggestName("dataArrayB1Way_" + x.toString)
    }
    ...
    for (i <- 0 until nWays) {
      ...
      if (refillsToOneBank) {
        ...
        mem_idx0 =
          Mux(refill_one_beat, (refill_idx << (log2Ceil(refillCycles) - 1)) | (refill_cnt >> 1.U), b0Row(s0_vaddr))
        mem_idx1 =
          Mux(refill_one_beat, (refill_idx << (log2Ceil(refillCycles) - 1)) | (refill_cnt >> 1.U), b1Row(s0_vaddr))
        ...
        mem_idx0 = Mux(refill_one_beat, (refill_idx << log2Ceil(refillCycles)) | refill_cnt, b0Row(s0_vaddr))
        mem_idx1 = Mux(refill_one_beat, (refill_idx << log2Ceil(refillCycles)) | refill_cnt, b1Row(s0_vaddr))
      ...
      tl_in.map { tl =>
        ...
        when(s2_slaveValid) {
          ...
          s1s3_slaveData := (0 until wordBits / 8)
            .map(i => Mux(byteEn(i), s2_dataCorrected, s1s3_slaveData)(8 * (i + 1) - 1, 8 * i))
            .asUInt
        }
