 ...
 */
class BoomFrontendBundle(val outer: BoomFrontend) extends CoreBundle()(outer.p) with HasExternallyDrivenTileConstants {
 ...
 * @param outer top level Frontend class
 */
class BoomFrontendModule(outer: BoomFrontend)
  ...
  s1_speculative := Mux(
    fetch_controller.io.imem_req.valid,
