 ...
 */
class OldFetchBuffer(numEntries: Int)(implicit p: Parameters)
    extends BoomModule
...
//    }
  ...
  val enq_count =
    Mux(
      io.enq.fire() && (!io.deq.ready || count =/= 0.U),
      popc_enqmask,
      Mux(io.enq.fire() && count === 0.U && popc_enqmask > coreWidth.U, popc_enqmask - coreWidth.U, 0.U)
    ) // !enq.fire || (count===0 and popc <= decodeWIdth)
  ...
  for (i <- 0 until fetchWidth) {
    when(io.enq.fire() && i.U < enq_count) {
      ...
      assert(
        !io.enq.fire() || ((start_idx + i.U) >= fetchWidth.U) || !compact_mask(start_idx + i.U),
  ...
  val deq_count =
    Mux(io.deq.ready, Mux(count < coreWidth.U, count, coreWidth.U), 0.U)
  ...
  if (DEBUG_PRINTF) {
    ...
    printf(
      "    Fetch3: Enq:(V:%c Msk:0x%x FIdx:%d CmptMsk:0x%x PC:0x%x EnqCnt:%d) Clear:%c\n",
    ...
    printf(
      "    Fetch4: Deq:(V:%c DeqCnt:%d PC:0x%x)\n",
