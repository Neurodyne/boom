 ...
 */
class FetchMonitor(implicit p: Parameters) extends BoomModule with HasBoomCoreParameters {
    ...
    when(prev_valid && uop.valid && io.fire) {
      when(prev_cfitype === CfiType.none) {
        assert(uop.bits.pc === prev_npc, "[fetchmonitor] non-cfi went to bad next-pc.")
          ...
          assert(uop.bits.pc === prev_npc || uop.bits.pc === prev_target, "[fetchmonitor] branch went to bad next-pc.")
        }
  ...
  when(io.fire) {
    ...
    val valid_mask = VecInit(io.uops map { u =>
      u.valid
    }).asUInt
    ...
    when(last_valid) {
      ...
      when(last_cfitype === CfiType.none) {
        when(first_pc =/= last_npc) {
          printf("  first_pc: 0x%x last_npc: 0x%x  ", first_pc, last_npc)
        }
        assert(first_pc === last_npc, "[fetchmonitor] A non-cfi instruction is followed by the wrong instruction.")
          ...
          assert(f_pc === targ, "[fetchmonitor] A jump is followed by the wrong instruction.")
        }
        ...
        .otherwise {
          ...
          assert(last_cfitype === CfiType.jalr, "[fetchmonitor] Should be a JALR if none of the others were valid.")
        }
    }
  }
