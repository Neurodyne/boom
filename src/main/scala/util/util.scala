...
object GetNewUopAndBrMask {
  def apply(uop: MicroOp, brinfo: BrResolutionInfo)(implicit p: Parameters): MicroOp = {
    val newuop = WireInit(uop)
    newuop.br_mask := Mux(brinfo.valid, (uop.br_mask & ~brinfo.mask), uop.br_mask)
    newuop
  }
}
...
object GetNewBrMask {
  def apply(brinfo: BrResolutionInfo, uop: MicroOp): UInt =
    return Mux(brinfo.valid, (uop.br_mask & ~brinfo.mask), uop.br_mask)

  def apply(brinfo: BrResolutionInfo, br_mask: UInt): UInt =
    return Mux(brinfo.valid, (br_mask & ~brinfo.mask), br_mask)
}
...
object WrapSub {
  ...
  def apply(value: UInt, amt: Int, n: Int): UInt =
    if (isPow2(n)) {
      ...
      Mux(value >= amt.U, value - amt.U, n.U - amt.U - value)
    }
}
 ...
 */
object ImmGen {
  import boom.common.{ IS_B, IS_I, IS_J, IS_S, IS_U, LONGEST_IMM_SZ }
  def apply(ip: UInt, isel: UInt): SInt = {
    ...
    val i11    = Mux(isel === IS_U, 0.S, Mux(isel === IS_J || isel === IS_B, ip(8).asSInt, sign))
 ...
 */
class BranchKillableQueue[T <: boom.common.HasBoomUOP](gen: T, entries: Int)(implicit p: Parameters)
