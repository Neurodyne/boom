...
trait HasBoomCoreIO extends freechips.rocketchip.tile.HasTileParameters {
  ...
  val io = new freechips.rocketchip.tile.CoreBundle with freechips.rocketchip.tile.HasExternallyDrivenTileConstants {
    ...
    val trace      = Output(Vec(coreParams.retireWidth, new freechips.rocketchip.rocket.TracedInstruction))
 ...
 */
class BoomCore(implicit p: Parameters, edge: freechips.rocketchip.tilelink.TLEdgeOut)
    extends BoomModule
    with HasBoomCoreIO {
  //**********************************
  ...
  val iregfile = if (enableCustomRf) {
    Module(
      new RegisterFileSeqCustomArray(
        numIntPhysRegs,
        numIrfReadPorts,
        numIrfWritePorts + 1, // + 1 for ll writebacks
        xLen,
        Seq(true) ++ exe_units.bypassable_write_port_mask
      )
    ) // 0th is bypassable ll_wb
  } else {
    Module(
  ...
  val btbStr =
    if (enableBTB)
      ("" + boomParams.btb.nSets * boomParams.btb.nWays + " entries (" + boomParams.btb.nSets + " x " + boomParams.btb.nWays + " ways)")
    else 0
  ...
  io.ifu.flush_pc := Mux(RegNext(RegNext(csr.io.eret)), RegNext(RegNext(csr.io.evec)), RegNext(csr.io.evec))
  ...
  io.ifu.flush_icache :=
    Range(0, coreWidth).map { i =>
      rob.io.commit.valids(i) && rob.io.commit.uops(i).is_fencei
    }.reduce(_ | _) ||
  ...
  for (w <- 0 until coreWidth) {
    ...
    val prev_insts_in_bundle_valid = Range(0, w).map { i =>
      dec_valids(i)
  ...
  for (w <- 0 until coreWidth) {
    ...
    if (coreWidth == 1) {
      ...
      dec_uops(w).rob_idx := Cat(rob.io.rob_tail_idx >> log2Ceil(coreWidth).U, w.U(log2Ceil(coreWidth).W))
    }
  }
  ...
  when(
    RegNext(!sxt_ldMiss) && RegNext(RegNext(lsu.io.mem_ldSpecWakeup.valid)) &&
    ...
    assert(
      mem_resp.valid && mem_resp.bits.uop.ctrl.rf_wen && mem_resp.bits.uop.dst_rtype === RT_FIX,
  ...
  csr.io.tval := Mux(
    tval_valid,
    encodeVirtualAddress(rob.io.com_xcpt.bits.badvaddr, rob.io.com_xcpt.bits.badvaddr),
    0.U
  )
...
//   "CSRFile is being written to spuriously.")
  ...
  assert(
    (dec_will_fire zip rename_stage.io.ren1_mask map { case (d, r) => d === r }).reduce(_ | _),
  ...
  if (usingFPU) {
    for ((wdata, wakeup) <- fp_pipeline.io.debug_wb_wdata zip fp_pipeline.io.wakeups) {
      ...
      assert(!(wakeup.valid && !wakeup.bits.uop.fp_val), "[core] FP wakeup does not involve an FP instruction.")
    }
  }
  ...
  assert(
    !(RegNext(rob.io.com_xcpt.valid) && !rob.io.flush.valid),
  ...
  if (DEBUG_PRINTF) {
    ...
    for (w <- 0 until coreWidth) {
      printf(
        "    Slot:%d (PC:0x%x Valids:%c%c Inst:DASM(%x))\n",
        w.U,
        dec_uops(w).pc(19, 0),
    ...
    for (w <- 0 until coreWidth) {
      printf(
        "    Slot:%d (PC:0x%x Valid:%c Inst:DASM(%x))\n",
        w.U,
    ...
    for (w <- 0 until coreWidth) {
      ...
      printf(
        "    Slot:%d (ISAREG: DST:%d SRCS:%d,%d,%d) (PREG: (#,Bsy,Typ) %d[-](%c) %d[%c](%c) %d[%c](%c) %d[%c](%c))\n",
        w.U,
        ren_uop.ldst,
        ren_uop.lrs1,
        ren_uop.lrs2,
        ...
        ren_uop.pdst,
        Mux(
          ren_uop.dst_rtype === RT_FIX,
          Str("X"),
          Mux(
            ren_uop.dst_rtype === RT_X,
            Str("-"),
            Mux(ren_uop.dst_rtype === RT_FLT, Str("f"), Mux(ren_uop.dst_rtype === RT_PAS, Str("C"), Str("?")))
          )
        ),
        ren_uop.prs1,
        BoolToChar(rename_stage.io.ren2_uops(w).prs1_busy, 'B', 'R'),
        Mux(
          ren_uop.lrs1_rtype === RT_FIX,
          Str("X"),
          Mux(
            ren_uop.lrs1_rtype === RT_X,
            Str("-"),
            Mux(ren_uop.lrs1_rtype === RT_FLT, Str("f"), Mux(ren_uop.lrs1_rtype === RT_PAS, Str("C"), Str("?")))
          )
        ),
        ren_uop.prs2,
        BoolToChar(rename_stage.io.ren2_uops(w).prs2_busy, 'B', 'R'),
        Mux(
          ren_uop.lrs2_rtype === RT_FIX,
          Str("X"),
          Mux(
            ren_uop.lrs2_rtype === RT_X,
            Str("-"),
            Mux(ren_uop.lrs2_rtype === RT_FLT, Str("f"), Mux(ren_uop.lrs2_rtype === RT_PAS, Str("C"), Str("?")))
          )
        ),
        ren_uop.prs3,
    ...
    if (DEBUG_PRINTF_ROB) {
      ...
      printf("ROB:\n")
      printf(
        "    (State:%c%c%c Rdy:%c LAQFull:%c STQFull:%c Flush:%c BMskFull:%c DShimRdy:%c) BMsk:0x%x Mode:%c\n",
        robTypeStrs(0),
        robTypeStrs(1),
        robTypeStrs(2),
        ...
        Mux(
          ...
          Str("M"),
          Mux(csr.io.status.prv === (PRV.U).U, Str("U"), Mux(csr.io.status.prv === (PRV.S).U, Str("S"), Str("?")))
        )
      )
    }
    ...
    printf("Other:\n")
    printf(
      "    Expt:(V:%c Cause:%d) Commit:%x IFreeLst:0x%x TotFree:%d IPregLst:0x%x TotPreg:%d\n",
    ...
    printf(
      "    FFreeList:0x%x TotFree:%d FPrefLst:0x%x TotPreg:%d\n",
    ...
    printf("Branch Unit:\n")
    printf(
      "    V:%c Mispred:%c T/NT:%c NPC:(V:%c PC:0x%x)\n",
  ...
  if (COMMIT_LOG_PRINTF) {
    ...
    for (w <- 0 until coreWidth) {
      ...
      when(rob.io.commit.valids(w)) {
        printf("%d 0x%x ", priv, Sext(rob.io.commit.uops(w).pc(vaddrBits - 1, 0), xLen))
  ...
  if (O3PIPEVIEW_PRINTF) {
    ...
    for (i <- 0 until coreWidth) {
      when(rob.io.commit.valids(i)) {
        printf("%d; O3PipeView:retire:%d:store: 0\n", rob.io.commit.uops(i).debug_events.fetch_seq, debug_tsc_reg)
      }
    }
  }
  ...
  if (usingRoCC) {
    ...
    for (w <- 0 until coreWidth) {
      exe_units.rocc_unit.io.rocc.dec_rocc_vals(w) := (dec_will_fire(w) &&
      rename_stage.io.inst_can_proceed(w) &&
      !rob.io.flush.valid &&
