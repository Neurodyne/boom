...
class RegisterFileSeqCustomArray(
  ...
  for (i <- 0 until numRegisters) {
    if (i == 0) {
      ...
      // setup reads
      regfile.io.OE(i) := VecInit(raddr_OH.map { item =>
        item(i)
      }).asUInt // read from a particular register

      // setup writes
      write_select_OH(i) := VecInit((waddr_OH zip io.write_ports).map { case (item, wp) => item(i) && wp.valid }).asUInt // write to a register
    ...
    if (i > 0) {
      ...
      assert(
        (PopCount(write_select_OH(i).asBools) <= 1.U) || (i.U === 0.U),
  ...
  if (bypassableArray.reduce(_ || _)) {
    ...
    io.write_ports zip bypassableArray map {
      case (wport, b) =>
        if (b) {
          bypassable_wports += wport
        }
    }

    for (i <- 0 until numReadPorts) {
      // bypass if matches with the read port input
      val bypass_enables = bypassable_wports.map(
        x =>
          x.valid && x.bits.addr =/= 0.U &&
      ...
      assert(
        PopCount(VecInit(bypass_enables)) <= 1.U,
 ...
 */
class RegisterFileArray(val numRegisters: Int, val numReadPorts: Int, val numWritePorts: Int, val registerWidth: Int)
    extends BlackBox
...
class RegisterFileArrayModel(
  ...
  for (i <- 0 until numRegisters) yield {
    val register = Module(new RegFileRegisterModel(numReadPorts, numWritePorts, registerWidth))
  ...
  for (rPortIdx <- 0 until numReadPorts) {
    assert(
      PopCount(io.OE.map(e => e(rPortIdx))) <= 1.U,
 ...
 */
class RegFileRegisterModel(numReadPorts: Int, numWritePorts: Int, registerWidth: Int) extends Module {
  val io = IO(new Bundle {
    val we = Input(Bool())
  ...
  for (i <- 0 until registerWidth) yield {
    val bit = Module(new RegFileBitModel(numReadPorts, numWritePorts))
    bit.io.we := io.we
    bit.io.ws := io.ws
    bit.io.wd := VecInit(io.wd.map { wPortData =>
      wPortData(i)
    })
    bit.io.oe := io.oe
  ...
  io.rd := rd.map { rPortDataBools =>
    rPortDataBools.asUInt
  }
}
 ...
 */
class RegFileBitModel(numReadPorts: Int, numWritePorts: Int) extends Module {
  val io = IO(new Bundle {
    val we = Input(Bool())
    val ws = Input(UInt(log2Ceil(numWritePorts).W))
  ...
  assert(!(io.we && io.ws >= numWritePorts.U), "[custom_regfile_bit] write-select set to invalid value.")
