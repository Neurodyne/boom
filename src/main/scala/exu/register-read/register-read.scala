...
class RegisterRead(
  ...
  for (w <- 0 until issueWidth) {
    ...
    rrd_valids(w) := RegNext(
      rrd_decode_unit.io.rrd_valid &&
  ...
  for (w <- 0 until issueWidth) {
    ...
    val rrd_kill = Mux(
      io.kill,
      true.B,
      Mux(io.brinfo.valid && io.brinfo.mispredict, maskMatch(rrd_uops(w).br_mask, io.brinfo.mask), false.B)
    )
  ...
  for (w <- 0 until issueWidth) {
    ...
    for (b <- 0 until io.bypass.getNumPorts) {
      ...
      rs1_cases ++= Array(
        (
          io.bypass.valid(b) && (prs1 === io.bypass.uop(b).pdst) && io.bypass.uop(b).ctrl.rf_wen
            && io.bypass.uop(b).dst_rtype === RT_FIX && lrs1_rtype === RT_FIX && (prs1 =/= 0.U),
          io.bypass.data(b)
        )
      )
      rs2_cases ++= Array(
        (
          io.bypass.valid(b) && (prs2 === io.bypass.uop(b).pdst) && io.bypass.uop(b).ctrl.rf_wen
            && io.bypass.uop(b).dst_rtype === RT_FIX && lrs2_rtype === RT_FIX && (prs2 =/= 0.U),
          io.bypass.data(b)
        )
      )
    }
