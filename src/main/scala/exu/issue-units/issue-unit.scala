 ...
 */
class IssueUnitIO(val issueWidth: Int, val numWakeupPorts: Int, val dispatchWidth: Int)(implicit p: Parameters)
    extends BoomBundle {
...
abstract class IssueUnit(
  ...
  for (w <- 0 until dispatchWidth) {
    ...
    if (iqType == IQT_MEM.litValue || iqType == IQT_INT.litValue) {
      ...
      when(
        (io.dis_uops(w).bits.uopc === uopSTA && io.dis_uops(w).bits.lrs2_rtype === RT_FIX) ||
  ...
  assert(
    !(io.sxt_ldMiss && !RegNext(io.mem_ldSpecWakeup.valid, init = false.B) && brKills === 0.U),
  ...
  if (O3PIPEVIEW_PRINTF) {
    for (i <- 0 until issueWidth) {
      ...
      when(io.iss_valids(i) && io.iss_uops(i).uopc =/= uopSTD) {
        printf("%d; O3PipeView:issue: %d\n", io.iss_uops(i).debug_events.fetch_seq, io.tsc_reg)
      }
    }
  }
  ...
  if (DEBUG_PRINTF_IQ) {
    ...
    for (i <- 0 until numIssueSlots) {
      printf(
        ...
        Mux(
          issue_slots(i).uop.dst_rtype === RT_FIX,
          Str("X"),
          Mux(
            issue_slots(i).uop.dst_rtype === RT_X,
            Str("-"),
            Mux(
              issue_slots(i).uop.dst_rtype === RT_FLT,
              Str("f"),
