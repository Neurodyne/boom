  ...
  val table: Array[(BitPat, List[BitPat])] =
    Array( //  |       |       |       |  |     |  |  |  |  |  |      |    |  |  |  |  |  |  |  |
      ...
      FENCE -> List(
        ...
        Y,
        CSR.N
      ), // TODO PERF make fence higher performance
...
object FDecode extends DecodeConstants {
  val table: Array[(BitPat, List[BitPat])] = Array(
    ...
    FSW -> List(
      ...
      N,
      CSR.N
    ), // sort of a lie; broken into two micro-ops
    FSD -> List(
      Y,
      Y,
      N,
 ...
 */
class DecodeUnit(implicit p: Parameters)
    extends BoomModule
    with freechips.rocketchip.rocket.constants.MemoryOpConstants {
...
//     cs.div && !csr.io.status.isa('m'-'a') || TODO check for illegal div instructions
  ...
  val (xcpt_valid, xcpt_cause) = checkExceptions(
    List(
...
class BranchDecode(implicit p: Parameters) extends BoomModule {
  ...
  val bpd_csignals =
    freechips.rocketchip.rocket.DecodeLogic(
      io.inst,
...
////                      //   |  |  |  |
  ...
  io.target := Mux(cs_is_br, ComputeBranchTarget(io.pc, io.inst, xLen), ComputeJALTarget(io.pc, io.inst, xLen))
  io.cfi_type :=
    Mux(cs_is_jalr, CfiType.jalr, Mux(cs_is_jal, CfiType.jal, Mux(cs_is_br, CfiType.branch, CfiType.none)))
}
