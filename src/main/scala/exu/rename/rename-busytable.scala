 ...
 */
class BusyTableIo(val plWidth: Int, val numPregs: Int, val numReadPorts: Int, val numWbPorts: Int)(
 ...
 * @param numWbPorts number of writeback ports to the regfile
 */
class BusyTableHelper(plWidth: Int, numPregs: Int, numReadPorts: Int, numWbPorts: Int)(implicit p: Parameters)
    extends BoomModule {
 ...
 * @param numWbPorts number of writeback ports to the regfile
 */
class BusyTable(plWidth: Int, rtype: BigInt, numPregs: Int, numReadPorts: Int, numWbPorts: Int)(implicit p: Parameters)
    extends BoomModule {
  val pregSz = log2Ceil(numPregs)
  ...
  val busy_table = Module(
    new BusyTableHelper(plWidth = plWidth, numPregs = numPregs, numReadPorts = numReadPorts, numWbPorts = numWbPorts)
  )
  ...
  for {
    ...
    when(
      io.ren_uops(w).lrs1_rtype === rtype.U && io.ren_will_fire(xx) && io.ren_uops(xx).ldst_val &&
        io.ren_uops(xx).dst_rtype === rtype.U && (io.ren_uops(w).lrs1 === io.ren_uops(xx).ldst)
    ) { prs1_was_bypassed(w) := true.B }
    when(
      io.ren_uops(w).lrs2_rtype === rtype.U && io.ren_will_fire(xx) && io.ren_uops(xx).ldst_val &&
    ...
    if (rtype == RT_FLT.litValue) {
      when(
        io.ren_uops(w).frs3_en && io.ren_will_fire(xx) && io.ren_uops(xx).ldst_val &&
