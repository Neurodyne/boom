 ...
 * @param numFpWbPorts number of FP writeback ports
 */
class RenameStage(plWidth: Int, numIntWbPorts: Int, numFpWbPorts: Int)(implicit p: Parameters) extends BoomModule {
  val io = IO(new RenameStageIO(plWidth, numIntPhysRegs, numFpPhysRegs, numIntWbPorts, numFpWbPorts))

  // integer registers
  val imaptable = Module(new RenameMapTable(plWidth, RT_FIX.litValue, 32, numIntPhysRegs))
  val ifreelist = Module(new RenameFreeList(plWidth, RT_FIX.litValue, numIntPhysRegs))
  val ibusytable = Module(
    new BusyTable(
      plWidth,
      RT_FIX.litValue,
  ...
  if (usingFPU) {
    fmaptable = Module(new RenameMapTable(plWidth, RT_FLT.litValue, 32, numFpPhysRegs))
    ffreelist = Module(new RenameFreeList(plWidth, RT_FLT.litValue, numFpPhysRegs))
    fbusytable = Module(
      new BusyTable(
        plWidth,
        RT_FLT.litValue,
  ...
  val ren2_imapvalues =
    if (renameLatency == 2) RegEnable(imaptable.io.values, ren2_will_proceed)
    else imaptable.io.values
  val ren2_fmapvalues =
    if (renameLatency == 2 && usingFPU) RegEnable(fmaptable.io.values, ren2_will_proceed)
  ...
  assert(
    !(io.int_wakeups.map(x => x.valid && x.bits.uop.dst_rtype =/= RT_FIX).reduce(_ | _)),
  ...
  if (usingFPU) {
    ...
    assert(
      !(io.fp_wakeups.map(x => x.valid && x.bits.uop.dst_rtype =/= RT_FLT).reduce(_ | _)),
  ...
  io.ren2_uops := ren2_uops map { u =>
    GetNewUopAndBrMask(u, io.brinfo)
  }
