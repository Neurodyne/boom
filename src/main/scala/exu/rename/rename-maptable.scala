 ...
 */
class RenameMapTableElement(val plWidth: Int, val always_zero: Boolean, val pregSz: Int)(implicit p: Parameters)
    extends BoomModule {
...
class RenameMapTable(
  ...
  for (w <- 0 until plWidth) {
    ...
    if (rtype == RT_FIX.litValue) {
      assert(
        !(io.ren_uops(w).lrs1 === 0.U && io.ren_uops(w).lrs1_rtype === RT_FIX && io.values(w).prs1 =/= 0.U),
        "lrs1==0 but maptable(" + w + ") returning non-zero."
      )
      assert(
        !(io.ren_uops(w).lrs2 === 0.U && io.ren_uops(w).lrs2_rtype === RT_FIX && io.values(w).prs2 =/= 0.U),
        "lrs2==0 but maptable(" + w + ") returning non-zero."
      )
      assert(
        !(io.ren_uops(w).lrs1 === 0.U && io.ren_uops(w).lrs1_rtype === RT_FIX && map_table_prs1(w) =/= 0.U),
        "lrs1==0 but maptable(" + w + ") returning non-zero."
      )
      assert(
        !(io.ren_uops(w).lrs2 === 0.U && io.ren_uops(w).lrs2_rtype === RT_FIX && map_table_prs2(w) =/= 0.U),
        "lrs2==0 but maptable(" + w + ") returning non-zero."
      )
    }
    // scalastyle:on
