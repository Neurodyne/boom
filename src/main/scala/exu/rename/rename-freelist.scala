 ...
 */
class FreeListIo(val numPregs: Int, val plWidth: Int)(implicit p: Parameters) extends BoomBundle {
 ...
 * @param plWidth pipeline width (dispatch group size)
 */
class RenameFreeListHelper(numPregs: Int, plWidth: Int)(implicit p: Parameters) extends BoomModule {
  ...
  for (w <- plWidth - 1 to 0 by -1) {
    ...
    just_allocated_mask =
      Mux(io.req_preg_vals(w) && allocated(w), requested_pregs_oh(w) | just_allocated_mask, just_allocated_mask)
  }
 ...
 * @param numPregs number of physical registers
 */
class RenameFreeList(plWidth: Int, rtype: BigInt, numPregs: Int)(implicit p: Parameters) extends BoomModule {
  val pregSz = log2Ceil(numPregs)
  ...
  val freelist = Module(new RenameFreeListHelper(numPregs, plWidth))
  ...
  when(io.debug_rob_empty) {
    assert(PopCount(freelist.io.debug.freelist) >= (numPregs - 32).U, "[freelist] We're leaking physical registers")
  }
}
