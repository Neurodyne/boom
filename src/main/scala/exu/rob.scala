...
object FlushTypes {
  ...
  def getType(valid: Bool, i_xcpt: Bool, i_eret: Bool, i_refetch: Bool): UInt = {
    val ret =
      Mux(!valid, none, Mux(i_eret, eret, Mux(i_xcpt, xcpt, Mux(i_refetch, refetch, next))))
    ret
  }
}
    ...
    for (i <- 0 until numWakeupPorts) {
      ...
      when(wb_resp.valid && MatchBank(GetBankIdx(wb_uop.rob_idx))) {
        ...
        if (O3PIPEVIEW_PRINTF) {
          printf("%d; O3PipeView:complete:%d\n", rob_uop(row_idx).debug_events.fetch_seq, io.debug_tsc)
        }
      }
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
    ...
    when(io.lxcpt.valid && MatchBank(GetBankIdx(io.lxcpt.bits.uop.rob_idx))) {
      ...
      when(io.lxcpt.bits.cause =/= MINI_EXCEPTION_MEM_ORDERING) {
        ...
        assert(
          rob_unsafe(GetRowIdx(io.lxcpt.bits.uop.rob_idx)),
    ...
    when(io.bxcpt.valid && MatchBank(GetBankIdx(io.bxcpt.bits.uop.rob_idx))) {
      ...
      assert(rob_unsafe(GetRowIdx(io.bxcpt.bits.uop.rob_idx)), "An instruction marked as safe is causing an exception")
    ...
    for (i <- 0 until numWakeupPorts) {
      ...
      assert(
        !(io.wb_resps(i).valid && MatchBank(GetBankIdx(rob_idx)) &&
  ...
  val insn_sys_pc2epc =
    rob_head_vals.reduce(_ | _) && PriorityMux(rob_head_vals, io.commit.uops.map { u =>
      u.is_sys_pc2epc
    })
  ...
  val flush_commit_mask = Range(0, coreWidth).map { i =>
    io.commit.valids(i) && io.commit.uops(i).flush_on_commit
  }
  val flush_commit = flush_commit_mask.reduce(_ | _)
  val flush_val    = exception_thrown || flush_commit

  assert(!(PopCount(flush_commit_mask) > 1.U), "[rob] Can't commit multiple flush_on_commit instructions on one cycle")
  ...
  io.flush.bits.flush_typ := RegNext(
    FlushTypes.getType(
      flush_val,
  ...
  when(!(io.flush.valid || exception_thrown) && rob_state =/= s_rollback) {
    when(io.lxcpt.valid || io.bxcpt.valid) {
      ...
      val idx = enq_xcpts.indexWhere { i: Bool =>
        i
      }
      ...
      assert(
        !(usingCompressed.B && (io.enq_uops(idx).uopc === uopJAL) && !io.enq_uops(idx).exc_cause.orR),
  ...
  assert(!(exception_thrown && !r_xcpt_val), "ROB trying to throw an exception, but it doesn't have a valid xcpt_cause")

  assert(!(empty && r_xcpt_val), "ROB is empty, but believes it has an outstanding exception.")

  assert(
    !(will_throw_exception && (GetRowIdx(r_xcpt_uop.rob_idx) =/= rob_head)),
  ...
  if (DEBUG_PRINTF_ROB) {
    printf("ROB:\n")
    printf(
      "    Xcpt: V:%c Cause:0x%x RobIdx:%d BMsk:0x%x BadVAddr:0x%x\n",
    ...
    for (i <- 0 until (numRobEntries / coreWidth)) {
      ...
      printf(
        ...
        row.U(robAddrSz.W),
        Mux(
          r_head === row.U && r_tail === row.U,
          Str("B"),
          Mux(r_head === row.U, Str("H"), Mux(r_tail === row.U, Str("T"), Str(" ")))
        ),
      ...
      if (coreWidth == 1) {
        ...
        printf(
          "(%c%c)(%c%c)(%c%c) 0x%x %x [DASM(%x)][DASM(%x)" + "] %c,%c %d,%d ",
          BoolToChar(debug_entry(r_idx + 0).valid, 'V'),
        ...
        val row_is_val = debug_entry(r_idx + 0).valid || debug_entry(r_idx + 1).valid || debug_entry(r_idx + 2).valid || debug_entry(
          r_idx + 3
        ).valid
        printf(
          "(%c%c%c%c)(%c%c%c%c)(%c%c%c%c) 0x%x %x %x %x [DASM(%x)][DASM(%x)][DASM(%x)][DASM(%x)" + "]%c%c%c%c",
          BoolToChar(debug_entry(r_idx + 0).valid, 'V'),
          BoolToChar(debug_entry(r_idx + 1).valid, 'V'),
      ...
      for (w <- 0 until coreWidth) {
        printf(
          "(d:%c p%d, bm:%x sdt:%d) ",
          Mux(
            debug_entry(temp_idx).uop.dst_rtype === RT_FIX,
            Str("X"),
            Mux(
              debug_entry(temp_idx).uop.dst_rtype === RT_PAS,
              Str("C"),
              Mux(
                debug_entry(temp_idx).uop.dst_rtype === RT_FLT,
                Str("f"),
