...
class FPU(implicit p: Parameters) extends BoomModule with tile.HasFPUParameters {
  ...
  val fpiu_out    = Pipe(RegNext(fpiu.io.in.valid && !fp_ctrl.fastpipe), fpiu.io.out.bits, fpu_latency - 1)
  ...
  val fpu_out_data =
    Mux(
      dfma.io.out.valid,
      box(dfma.io.out.bits.data, true.B),
      Mux(
        sfma.io.out.valid,
        box(sfma.io.out.bits.data, false.B),
        Mux(fpiu_out.valid, fpiu_result.data, box(fpmu.io.out.bits.data, fpmu_double))
      )
    )
  ...
  val fpu_out_exc =
    Mux(
      dfma.io.out.valid,
      dfma.io.out.bits.exc,
      Mux(sfma.io.out.valid, sfma.io.out.bits.exc, Mux(fpiu_out.valid, fpiu_result.exc, fpmu.io.out.bits.exc))
    )
