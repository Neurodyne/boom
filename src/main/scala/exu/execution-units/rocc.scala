...
class RoCCShim(implicit p: Parameters) extends BoomModule {
  ...
  when(io.req.valid) {
    ...
    assert(
      io.req.bits.uop.rob_idx === rxq_uop(rxq_idx).rob_idx,
      "Mismatch between RoCCUnit request and RoCC execute head"
    )
    assert(rxq_val(rxq_idx), "Trying to execute rocc inst without the instruction bits")
  ...
  when(handle_resp) {
    assert(
      (rcq.io.deq.bits.dst_rtype === RT_X)
