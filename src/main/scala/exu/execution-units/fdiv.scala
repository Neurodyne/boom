...
class FDivSqrtUnit(implicit p: Parameters)
    extends FunctionalUnit(isPipelined = false, numStages = 1, numBypassStages = 0, dataWidth = 65, needsFcsr = true)
  ...
  assert(
    !(r_out_val && (divsqrt.io.outValid_div || divsqrt.io.outValid_sqrt)),
    "[fdiv] Buffered output being overwritten by another output from the fdiv/fsqrt unit."
  )

  val downvert_d2s = Module(
    new hardfloat.RecFNToRecFN(inExpWidth = 11, inSigWidth = 53, outExpWidth = 8, outSigWidth = 24)
  )
  ...
  io.resp.bits.data :=
    Mux(r_divsqrt_fin.singleIn, box(downvert_d2s.io.out, false.B), box(r_out_wdata_double, true.B))
