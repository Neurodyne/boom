...
import boom.bpu.{ BoomBTBUpdate, BpredType, BranchPredInfo }
 ...
 */
class FuncUnitReq(val dataWidth: Int)(implicit p: Parameters) extends BoomBundle with HasBoomUOP {
  val numOperands = 3
 ...
 */
class FuncUnitResp(val dataWidth: Int)(implicit p: Parameters) extends BoomBundle with HasBoomUOP {
...
abstract class FunctionalUnit(
  ...
  val needsFcsr: Boolean = false
    ...
    extends BoomModule {
  val io = IO(new FunctionalUnitIo(numStages, numBypassStages, dataWidth, isBranchUnit, needsFcsr))
}
...
class ALUUnit(isBranchUnit: Boolean = false, numStages: Int = 1, dataWidth: Int)(implicit p: Parameters)
  ...
  if (isBranchUnit) {
    ...
    op1_data = Mux(
      io.req.bits.uop.ctrl.op1_sel.asUInt === OP1_RS1,
      io.req.bits.rs1_data,
      Mux(io.req.bits.uop.ctrl.op1_sel.asUInt === OP1_PC, Sext(curr_pc, xLen), 0.U)
    )
  } else {
    op1_data = Mux(io.req.bits.uop.ctrl.op1_sel.asUInt === OP1_RS1, io.req.bits.rs1_data, 0.U)
  }

  // operand 2 select
  val op2_data = Mux(
    io.req.bits.uop.ctrl.op2_sel === OP2_IMM,
    Sext(imm_xprlen.asUInt, xLen),
    Mux(
      io.req.bits.uop.ctrl.op2_sel === OP2_IMMC,
      io.req.bits.uop.prs1(4, 0),
      Mux(
        io.req.bits.uop.ctrl.op2_sel === OP2_RS2,
        io.req.bits.rs2_data,
        Mux(io.req.bits.uop.ctrl.op2_sel === OP2_NEXT, Mux(io.req.bits.uop.is_rvc, 2.U, 4.U), 0.U)
      )
    )
  )
  ...
  if (isBranchUnit) {
    ...
    val pc_sel = MuxLookup(
      io.req.bits.uop.ctrl.br_type,
      PC_PLUS4,
      Seq(
        BR_N   -> PC_PLUS4,
    ...
    if (DEBUG_PRINTF) {
      printf("BR-UNIT:\n")
      printf(
        "    PC:0x%x+0x%x Next:(V:%c PC:0x%x) BJAddr:0x%x\n",
    ...
    when(io.req.valid && uop.is_jal && io.get_ftq_pc.next_val && io.get_ftq_pc.next_pc =/= bj_addr) {
      printf(
        "[func] JAL went to the wrong target [curr: 0x%x+%x next: 0x%x, target: 0x%x]",
        io.get_ftq_pc.fetch_pc,
        io.req.bits.uop.pc_lob,
        io.get_ftq_pc.next_pc,
        bj_addr
    ...
    assert(
      !(io.req.valid && uop.is_jal && io.get_ftq_pc.next_val && io.get_ftq_pc.next_pc =/= bj_addr),
    ...
    when(is_br_or_jalr && pc_sel === PC_BRJMP && !mispredict && io.get_ftq_pc.next_val) {
      ...
      when(
        io.get_ftq_pc.next_pc(vaddrBits - 1, log2Ceil(coreInstBytes)) =/=
    ...
    when(is_br_or_jalr) {
      ...
      when(pc_sel === PC_PLUS4) {
        mispredict :=
          Mux(uop.br_prediction.btb_blame, btb_mispredict, Mux(uop.br_prediction.bpd_blame, bpd_mispredict, false.B)) // if neither BTB nor BPD predicted and it's not-taken, then no misprediction occurred.
      }
      when(pc_sel === PC_BRJMP) {
        mispredict :=
          Mux(uop.br_prediction.btb_blame, btb_mispredict, Mux(uop.br_prediction.bpd_blame, bpd_mispredict, true.B)) // if neither BTB nor BPD predicted and it's taken, then a misprediction occurred.
      }
    }
    ...
    brinfo.cfi_type := Mux(
      uop.is_jal,
      CfiType.jal,
      Mux(pc_sel === PC_JALR, CfiType.jalr, Mux(uop.is_br_or_jmp, CfiType.branch, CfiType.none))
    )
    ...
    br_unit.btb_update.bits.cfi_idx := Mux(io.req.bits.uop.edge_inst, 0.U, (uop_pc_ >> log2Ceil(coreInstBytes)))
    ...
    br_unit.btb_update.bits.cfi_type :=
      Mux(uop.is_jal, CfiType.jal, Mux(uop.is_jump && !uop.is_jal, CfiType.jalr, CfiType.branch))
    br_unit.btb_update.bits.bpd_type :=
      Mux(
        uop.is_ret,
        BpredType.RET,
        Mux(uop.is_call, BpredType.CALL, Mux(uop.is_jump, BpredType.JUMP, BpredType.BRANCH))
      )
...
class MemAddrCalcUnit(implicit p: Parameters)
  ...
  if (dataWidth > 63) {
    assert(
      !(io.req.valid && io.req.bits.uop.ctrl.is_std &&
        io.resp.bits.data(64).asBool === true.B),
      "65th bit set in MemAddrCalcUnit."
    )

    assert(
      !(io.req.valid && io.req.bits.uop.ctrl.is_std && io.req.bits.uop.fp_val),
      "FP store-data should now be going through a different unit."
    )
  }

  assert(
    !(io.req.bits.uop.fp_val && io.req.valid && io.req.bits.uop.uopc =/=
  ...
  io.resp.bits.mxcpt.bits := Mux(
    ma_ld,
    freechips.rocketchip.rocket.Causes.misaligned_load.U,
...
class IntToFPUnit(latency: Int)(implicit p: Parameters)
  ...
  assert(
    !(io.req.valid && fp_ctrl.fromint && req.in1(xLen).asBool),
    "[func] IntToFP integer input has 65th high-order bit set!"
  )

  assert(!(io.req.valid && !fp_ctrl.fromint), "[func] Only support fromInt micro-ops.")
 ...
 */
class DivUnit(dataWidth: Int)(implicit p: Parameters) extends IterativeFunctionalUnit(dataWidth) {
