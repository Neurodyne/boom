...
import boom.util.{ BoomCoreStringPrefix, BranchKillableQueue, ImmGen, IsKilledByBranch }
 ...
 */
class ExeUnitResp(val dataWidth: Int)(implicit p: Parameters) extends BoomBundle with HasBoomUOP {
...
abstract class ExecutionUnit(
  ...
  if (writesIrf) {
    io.iresp.bits.fflags.valid := false.B; assert(io.iresp.ready)
  }
  if (writesLlIrf) {
    io.ll_iresp.bits.fflags.valid := false.B
  }
  if (writesFrf) {
    io.fresp.bits.fflags.valid := false.B; assert(io.fresp.ready)
  }
  if (writesLlFrf) {
  ...
  require(
    (hasFpu || hasFdiv) ^ (hasAlu || hasMul || hasMem || hasIfpu),
...
class ALUExeUnit(
  ...
  require(!(hasRocc && !hasCSR), "RoCC needs to be shared with CSR unit")
  require(!(hasMem && hasRocc), "We do not support execution unit with both Mem and Rocc writebacks")
  require(!(hasMem && hasIfpu), "TODO. Currently do not support AluMemExeUnit with FP")
  ...
  if (hasAlu) {
    alu = Module(new ALUUnit(isBranchUnit = hasBrUnit, numStages = numBypassStages, dataWidth = xLen))
  ...
  if (hasIfpu) {
    ...
    val queue = Module(new BranchKillableQueue(new ExeUnitResp(dataWidth), entries = intToFpLatency + 3)) // TODO being overly conservative
  ...
  if (hasMem) {
    ...
    io.dmem.req.valid := Mux(io.com_exception && io.lsu_io.memreq_uop.is_load, false.B, io.lsu_io.memreq_val)
    ...
    //anything....
    val memresp_val =
      Mux(io.com_exception && io.dmem.resp.bits.uop.is_load, false.B, io.lsu_io.forward_val || io.dmem.resp.valid)
    ...
    val memresp_uop  = Mux(io.lsu_io.forward_val, io.lsu_io.forward_uop, io.dmem.resp.bits.uop)
  ...
  if (writesIrf) {
    ...
    io.iresp.bits.uop := PriorityMux(iresp_fu_units.map(f => (f.io.resp.valid, f.io.resp.bits.uop.asUInt)))
      .asTypeOf(new MicroOp())
    io.iresp.bits.data := PriorityMux(iresp_fu_units.map(f => (f.io.resp.valid, f.io.resp.bits.data.asUInt))).asUInt
  ...
  assert(
    (PopCount(iresp_fu_units.map(_.io.resp.valid)) <= 1.U && !div_resp_val) ||
...
class FPUExeUnit(
  ...
  io.fresp.bits.uop := PriorityMux(fu_units.map(f => (f.io.resp.valid, f.io.resp.bits.uop.asUInt)))
    .asTypeOf(new MicroOp())
  ...
  if (hasFpiu) {
    ...
    // buffer up results since we share write-port on integer regfile.
    val queue = Module(new BranchKillableQueue(new ExeUnitResp(dataWidth), entries = dfmaLatency + 3)) // TODO being overly conservative
