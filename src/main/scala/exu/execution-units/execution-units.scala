...
class ExecutionUnits(val fpu: Boolean)(implicit val p: Parameters) extends HasBoomCoreParameters {
  ...
  if (!fpu) {
    ...
    if (!usingUnifiedMemIntIQs) {
      val memExeUnit = Module(new ALUExeUnit(hasAlu = false, hasMem = true))
    ...
    for (w <- 0 until fp_width) {
      val fpu_exe_unit = Module(new FPUExeUnit(hasFpu = true, hasFdiv = usingFDivSqrt && (w == 0), hasFpiu = (w == 0)))
  ...
  override def toString: String =
    (BoomCoreStringPrefix("===ExecutionUnits===") + "\n"
      + (if (!fpu) {
           BoomCoreStringPrefix("==" + coreWidth + "-wide Machine==", "==" + totalIssueWidth + " Issue==")
         } else {
           ""
         }) + "\n"
  ...
  if (!fpu) {
    ...
    require(exe_units.map(_.hasFpu).reduce(_ | _), "Datapath is missing a fpu (or has an fpu and shouldnt).")
  }
