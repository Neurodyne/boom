...
trait HasBoomLazyRoCC extends CanHaveBoomPTW { this: BaseTile =>
  ...
  roccs.map(_.atlNode).foreach { atl =>
    tlMasterXbar.node :=* atl
  }
  roccs.map(_.tlNode).foreach { tl =>
    tlOtherMastersNode :=* tl
  }
 ...
 * Mixin for BOOM when it uses a RoCC accelerator
 */
trait HasBoomLazyRoCCModule extends CanHaveBoomPTWModule with HasCoreParameters { this: BoomTileModuleImp =>
