...
class BranchPredictionStage(implicit p: Parameters) extends BoomModule {
  ...
  btb.io.btb_update := Mux(io.br_unit_resp.btb_update.valid, io.br_unit_resp.btb_update, io.f3_btb_update)
  ...
  if (DEBUG_PRINTF) {
    printf("BPD Pipeline:\n")
    printf(
      "    BTB: F0NPC:(V:%c PC:0x%x) F2RESP:(V:%c TRG:0x%x)\n",
  ...
  when(io.f2_valid && btb.io.resp.valid) {
    assert(
      btb.io.resp.bits.fetch_pc(15, 0) === io.debug_imemresp_pc(15, 0),
