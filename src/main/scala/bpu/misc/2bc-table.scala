...
import boom.util.{ BoomCoreStringPrefix, SeqMem1rwTransformable }
...
class HTable(numPEntries: Int, shareHBit: Boolean)(implicit p: Parameters) extends BoomModule {
  ...
  h_table.io.wdata := VecInit(hwq.io.deq.bits.takens.map(t => Mux(hwq.io.deq.bits.do_initialize, !t, t))).asUInt
  ...
  io.pwq_enq.bits.new_value := Mux(RegNext(io.update.bits.do_initialize), RegNext(io.update.bits.takens.asUInt), h_rout)
}
...
class TwobcCounterTable(
  ...
  val p_table =
    if (dualported) Module(new PTableDualPorted(numEntries))
