 ...
 */
class TageTableIo(val idxSz: Int, val tagSz: Int, val cntrSz: Int, val ubitSz: Int)(implicit p: Parameters)
    extends BoomBundle {
...
class TageTable(
    ...
    next :=
      Mux(
        ...
        Mux(
          !taken && allocate,
          CNTR_WEAK_NOTTAKEN,
          Mux(taken && old =/= CNTR_MAX, old + 1.U, Mux(!taken && old =/= 0.U, old - 1.U, old))
        )
      )
    next
  }
  ...
  private def generateUBit(old: UInt, allocate: Bool, update: Bool, degrade: Bool, mispredicted: Bool): UInt = {
    ...
    require(ubitSz == 1)
    next :=
      Mux(allocate, UBIT_INIT_VALUE, Mux(update && !mispredicted, 1.U, 0.U))
