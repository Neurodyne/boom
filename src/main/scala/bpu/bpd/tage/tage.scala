...
import boom.util.{ BoomCoreStringPrefix, ElasticReg, Fold }
...
class TageBrPredictor(
  ...
  ubitSz: Int
    ...
    extends BoomBrPredictor(historyLength = historyLengths.max) {
  ...
  def GetAlternateTableId(hits: IndexedSeq[Bool]): (Bool, UInt) = {
    ...
    assert((PopCount(hits) > 1.U) ^ !found_second, "[Tage] GetAltId has a disagreement on finding a second hit.")
    (found_second, alt_id)
  }
...
//  f3_tag_hits(i) && io.f3_is_br(f3_predictions(i).cidx)
  ...
  for (w <- 0 until fetchWidth) yield {
    val found_first = f3_hits_matrix.map { mask =>
      mask(w)
    }.reduce(_ | _)
    ...
    // TODO allow use_alt
    f3_takens(w) :=
      Mux(f3_best_hits(w), VecInit(f3_predictions)(f3_best_ids(w)).predictsTaken, false.B)
  }
...
//  }
  ...
  val predicted_cidx =
    Mux(f3_takens.reduce(_ | _), PriorityEncoder(f3_takens), PriorityEncoder(f3_best_hits)) // TODO allow use_alt
  ...
  io.resp.bits.takens := Mux(f3_has_hit, GetPredictionOH(f3_pred.cidx, f3_pred.cntr), io.f2_bim_resp.bits.getTakens)
  ...
  val r_info = (r_commit.bits.info).asTypeOf(
    new TageResp(
      fetchWidth = fetchWidth,
      numTables = numTables,
      maxHistoryLength = historyLengths.max,
  ...
  val com_indexes = historyLengths zip tableSizes map {
    case (hlen, tsize) =>
  ...
  when(r_commit.valid) {
    ...
    when(r_commit.bits.mispredict && (r_info.provider_id < maxTableId.U || !r_info.provider_hit)) {
      ...
      val temp = Mux(rand === 3.U, 2.U, Mux(rand === 2.U, 1.U, 0.U))
      val ridx = Mux((Cat(0.U, r_info.provider_id) + temp) >= maxTableId.U, 0.U, temp)
