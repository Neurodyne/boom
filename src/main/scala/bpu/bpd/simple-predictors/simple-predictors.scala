...
import boom.util.{ BoomCoreStringPrefix, ElasticReg }
...
class NullBrPredictor(
  ...
  override def toString: String = BoomCoreStringPrefix("==Null BPU==", "Building (0 kB) Null Predictor (never predict)")
}
...
class RandomBrPredictor(
    ...
    extends BoomBrPredictor(historyLength = 1) {
  ...
  override def toString: String = BoomCoreStringPrefix("==Random BPU==", "Building Random Branch Predictor")
}
