...
abstract class BoomBrPredictor(val historyLength: Int)(implicit p: Parameters) extends BoomModule {
  ...
  f0_history := Mux(io.ftq_restore.valid, io.ftq_restore.bits.history, Mux(io.f4_redirect, r_f4_history, r_f1_history)) // valid for f2_redirect
  ...
  r_f1_history := Mux(
    ...
    Mux(
      ...
      Mux(io.f4_redirect && !io.f4_taken, r_f4_history, Mux(use_new_hash, new_history, r_f1_history))
    )
  )

  assert(
    !io.f2_redirect || r_f2_history === r_f1_history,
  ...
  def apply(boomParams: BoomCoreParams)(implicit p: Parameters): BoomBrPredictor = {
    ...
    if (enableCondBrPredictor) {
      if (useBaseOnly) {
        ...
        br_predictor = Module(new GShareBrPredictor(historyLength = boomParams.gshare.get.historyLength))
      } else if (useTage) {
        br_predictor = Module(
          new TageBrPredictor(
      ...
      br_predictor = Module(new NullBrPredictor(historyLength = 1))
    }
