...
abstract class BimBundle(implicit val p: Parameters)
    extends freechips.rocketchip.util.ParameterizedBundle
...
class BimodalTable(implicit p: Parameters) extends BoomModule with HasBimParameters {
  ...
  private def updateCounter(cntr: UInt, taken: Bool): UInt = {
    ...
    next :=
      Mux(taken && cntr =/= 3.U, cntr + 1.U, Mux(!taken && cntr =/= 0.U, cntr - 1.U, cntr))
    next
  }
  ...
  if (DEBUG_PRINTF) {
    printf("BIM:\n")
    printf(
      "    ReqPC:0x%x (LIdx:%d BnkIdx:%d Row:%d) S2RespIdx:%d\n",
      io.req.bits.addr,
      s0_logical_idx,
      s0_bank_idx,
  ...
  for (w <- 0 until nBanks) {
    ...
    if (DEBUG_PRINTF) {
      printf("    Bank[%d]: REN:%c ", w.U, BoolToChar(ren, 'R'))
    }
    ...
    when(wen) {
      ...
      if (DEBUG_PRINTF)
        printf("WEN:W WAddr:(%d=0x%x) Data:0x%x Msk:0x%x ", waddr, waddr, wdata.asUInt, VecInit(wmask).asUInt)
    }.otherwise {
    ...
    val rrow         = Mux(s0_rmw_valid, getRowFromIdx(uq.io.deq.bits.entry_idx), getRowFromIdx(s0_logical_idx))
    ...
    s2_rmw_data := updateCounterInRow(s2_read_out(w), s2_rmw_cfi_idx, RegNext(RegNext(uq.io.deq.bits.taken)))
    ...
    if (DEBUG_PRINTF) {
      printf(
        "UpdQ:(Enq:(V:%c R:%c) Deq:(V:%c R:%c)) S0_RMW:%c S2_OUT:0x%x RMW_DATA:0x%x WrQ:(EnqV:%d DeqV:%d)\n",
