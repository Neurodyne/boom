...
class DenseBTB(implicit p: Parameters) extends BoomBTB {
  ...
  private def getBankWriteData(next_way: UInt, btb_q_entry: BTBUpdateQueueEntry) = {
    ...
    when(level === 1.U) {
      ...
      wdata(1).offset := btb_q_entry.update
        .target(min(vaddrBits - 1, tagSz + 2 * offsetSz + lsbSz - 1), tagSz + offsetSz + lsbSz)
    ...
    if (branchLevels > 2) {
      when(level === 2.U) {
        for (i <- 0 until 1) {
          wdata(i).tag := btb_q_entry.update
            .target((i + 1) * (tagSz + offsetSz) + lsbSz - 1, i * tagSz + (i + 1) * offsetSz + lsbSz)
          wdata(i).offset := btb_q_entry.update
            .target(i * tagSz + (i + 1) * offsetSz + lsbSz - 1, i * (tagSz + offsetSz) + lsbSz)
        }
        wdata(2).tag := getTag(btb_q_entry.update.pc)
        wdata(2).offset := btb_q_entry.update
          .target(min(vaddrBits - 1, 2 * tagSz + 3 * offsetSz + lsbSz - 1), 2 * (tagSz + offsetSz) + lsbSz)
  ...
  for (b <- 0 until nBanks) {
    ...
    val rout = VecInit(rout_bits map { x =>
      x.asTypeOf(new BTBSetData())
    })
    ...
    when(!ren && wen) {
      ...
      val wdata_bits = VecInit(wdata map { x =>
        x.asUInt
      })
  ...
  val cfi_oh = ((0 until nWays).map { i =>
    Mux(hits(i), UIntToOH(data_out(i.U).cfi_idx, fetchWidth), 0.U(fetchWidth.W))
  ...
  when(blevel === 1.U) {
    ...
    val resp_target = if ((vaddrBits - lsbSz) <= tagSz + 2 * offsetSz + lsbSz) {
      Cat(
        data_out(1).offset((vaddrBits - 1) - (tagSz + offsetSz + lsbSz), 0),
        data_out(0).tag,
        data_out(0).offset,
        0.U(lsbSz.W)
      )
      ...
      Cat(
        s1_pc(vaddrBits - 1, tagSz + 2 * offsetSz + lsbSz),
        data_out(1).offset,
        data_out(0).tag,
        data_out(0).offset,
        0.U(lsbSz.W)
  ...
  if (branchLevels > 2) {
    when(blevel === 2.U) {
      ...
      val resp_target = if ((vaddrBits - lsbSz) <= 2 * tagSz + 3 * offsetSz + lsbSz) {
        Cat(
          data_out(2).offset((vaddrBits - 1) - (2 * (tagSz + offsetSz) + lsbSz), 0),
          data_out(1).tag,
          data_out(1).offset,
          data_out(0).tag,
          data_out(0).offset,
        ...
        Cat(
          s1_pc(vaddrBits - 1, 2 * tagSz + 3 * offsetSz + lsbSz),
          data_out(2).offset,
          data_out(1).tag,
          data_out(1).offset,
          data_out(0).tag,
  ...
  if (DEBUG_PRINTF) {
    printf(
      "BTB predi (%c): hits:%x %d (PC= 0x%x, TARG= 0x%x %d) s2_BIM [%d %d 0x%x]\n",
