...
class BTBsa(implicit p: Parameters) extends BoomBTB {
  ...
  for (w <- 0 until nWays) {
    ...
    if (DEBUG_PRINTF) {
      printf(
        "    Write (%c): (TAG[%d][%d] <- 0x%x) (PC:0x%x TARG:0x%x)\n",
        BoolToChar(wen, 'W'),
        w.U,
        widx,
        wtag,
  ...
  if (nRAS > 0) {
    ...
    val doPeek =
      (hits_oh zip data_out map { case (hit, data) => hit && BpredType.isReturn(data.bpd_type) }).reduce(_ || _)
  ...
  if (DEBUG_PRINTF) {
    ...
    printf(
      "    Predicted (%c): Hits:b%b (PC:0x%x -> TARG:0x%x) CFI:%c%c%c%c\n",
    ...
    printf(
      "    BIM: Predicted (%c): Idx:%d Row:0x%x\n",
